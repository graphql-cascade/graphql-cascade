...
async function checkConfiguration(result: DiagnosticResult): Promise<void> {
  ...
  const schemaFiles = ["schema.graphql", "schema.gql", "src/schema.graphql"];
...
async function checkTypeScriptVersion(result: DiagnosticResult): Promise<void> {
  try {
    ...
    if (dependencies["typescript"]) {
      ...
      if (
        majorVersion >= 4 &&
        parseInt(version.split(".")[1] || "0", 10) >= 7
      ) {
        ...
        result.warnings.push(
          `TypeScript ${version} may be outdated. Recommended: >=4.7`,
        );
      }
    } else {
      result.warnings.push(
  ...
  try {
    ...
    if (dependencies["graphql"]) {
      ...
      if (majorVersion >= 16) {
        ...
        result.errors.push(
          `GraphQL ${version} is not supported. Required: >=16.0.0. Fix: npm install graphql@^16.0.0`,
        );
      }
    } else {
      result.errors.push(
...
async function checkPeerDependencies(result: DiagnosticResult): Promise<void> {
  try {
    ...
    const clientChecks = [
      {
        cascade: "@graphql-cascade/client-apollo",
        peer: "@apollo/client",
        peerName: "Apollo Client",
      },
      {
        cascade: "@graphql-cascade/client-react-query",
        peer: "@tanstack/react-query",
        peerName: "React Query",
      },
      {
        cascade: "@graphql-cascade/client-urql",
        peer: "urql",
        peerName: "URQL",
      },
      {
        cascade: "@graphql-cascade/client-relay",
        peer: "react-relay",
        peerName: "Relay",
      },
    ];
    ...
    for (const check of clientChecks) {
      if (dependencies[check.cascade]) {
        if (dependencies[check.peer]) {
          ...
          result.errors.push(
            `${check.cascade} requires ${check.peer}. Fix: npm install ${check.peer}`,
          );
        }
      }
    }
