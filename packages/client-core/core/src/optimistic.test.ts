...
describe("OptimisticCascadeClient", () => {
  ...
  describe("mutateOptimistic", () => {
    it("should apply optimistic update immediately", async () => {
      ...
      const result = await client.mutateOptimistic<User>(
        {} as any,
        {},
        optimisticResponse,
      );
    ...
    it("should rollback on mutation error", async () => {
      ...
      await expect(
        client.mutateOptimistic({} as any, {}, optimisticResponse),
    ...
    it("should handle optimistic deletions and evict entities", async () => {
      ...
      expect(
        cache.evicted.some((e) => e.typename === "Post" && e.id === "1"),
      ).toBe(true);
    });
  });
  ...
  describe("rollback", () => {
    ...
    it("should handle entities that did not exist before", () => {
      ...
      expect(
        cache.written.some((w) => w.typename === "Post" && w.id === "999"),
      ).toBe(true);
      ...
      expect(
        cache.evicted.some((e) => e.typename === "Post" && e.id === "999"),
      ).toBe(true);
    });
  ...
  describe("applyOptimistic", () => {
    ...
    it("should handle multiple entities in optimistic update", () => {
      ...
      expect(
        cache.written.some((w) => w.typename === "User" && w.id === "1"),
      ).toBe(true);
      expect(
        cache.written.some((w) => w.typename === "Post" && w.id === "2"),
      ).toBe(true);
    });
  });
  ...
  describe("multiple optimistic updates", () => {
    it("should stack multiple optimistic updates correctly", async () => {
      ...
      const promise1 = client.mutateOptimistic(
        {} as any,
        {},
        optimisticResponse1,
      );
      ...
      const promise2 = client.mutateOptimistic(
        {} as any,
        {},
        optimisticResponse2,
      );
...
describe("CascadeConflictResolver", () => {
  ...
  describe("edge cases", () => {
    ...
    it("should handle MANUAL resolution strategy by throwing", () => {
      ...
      expect(() => resolver.resolveConflicts(conflict, "MANUAL")).toThrow(
