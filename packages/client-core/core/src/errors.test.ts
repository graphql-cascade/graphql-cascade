...
describe("Error Classification", () => {
  ...
  describe("isAuthError", () => {
    it("should identify authentication errors", () => {
      ...
      expect(
        isAuthError({ code: CascadeErrorCode.FORBIDDEN, message: "forbidden" }),
      ).toBe(true);
    });
    ...
    it("should identify non-authentication errors", () => {
      ...
      expect(
        isAuthError({ code: CascadeErrorCode.NOT_FOUND, message: "not found" }),
      ).toBe(false);
    });
  });
  ...
  describe("isClientError", () => {
    it("should identify client errors", () => {
      ...
      expect(
        isClientError({ code: CascadeErrorCode.CONFLICT, message: "conflict" }),
      ).toBe(true);
    });
    ...
    it("should identify server errors", () => {
      ...
      expect(
        isClientError({ code: CascadeErrorCode.TIMEOUT, message: "timeout" }),
      ).toBe(false);
      expect(
        isClientError({
          code: CascadeErrorCode.SERVICE_UNAVAILABLE,
...
describe("Retry Logic", () => {
  describe("calculateRetryDelay", () => {
    ...
    it("should cap exponential backoff at maxDelay", () => {
      const error = { code: CascadeErrorCode.TIMEOUT, message: "timeout" };
      expect(
        calculateRetryDelay(error, 10, { baseDelay: 1000, maxDelay: 5000 }),
      ).toBe(5000);
    });

    it("should use fixed delay when exponentialBackoff is false", () => {
      const error = { code: CascadeErrorCode.TIMEOUT, message: "timeout" };
      expect(calculateRetryDelay(error, 1, { exponentialBackoff: false })).toBe(
        1000,
      );
      expect(calculateRetryDelay(error, 2, { exponentialBackoff: false })).toBe(
        1000,
      );
      expect(calculateRetryDelay(error, 3, { exponentialBackoff: false })).toBe(
        1000,
      );
    });
