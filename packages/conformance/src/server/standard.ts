 ...
 */
async function runCascadeDepthControlTests(
  options: ServerConformanceOptions,
): Promise<TestCategory> {
  ...
  try {
    ...
    const passed =
      response.cascade.metadata.depth === 1 &&
      response.cascade.updated.length === 1;
    tests.push({
      name: "Depth 1 returns primary only",
      passed,
      message: passed
        ? "Depth 1 correctly returns only primary entity"
        : `Expected depth 1 with 1 entity, got depth ${response.cascade.metadata.depth} with ${response.cascade.updated.length} entities`,
    });
  } catch (error) {
  ...
  try {
    ...
    const passed =
      response.cascade.metadata.depth === 2 &&
      response.cascade.updated.length >= 2;
    tests.push({
      name: "Depth 2 returns related",
      passed,
      message: passed
        ? "Depth 2 correctly returns primary and related entities"
        : `Expected depth 2 with >=2 entities, got depth ${response.cascade.metadata.depth} with ${response.cascade.updated.length} entities`,
    });
  } catch (error) {
    tests.push({
      name: "Depth 2 returns related",
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "Depth correctly respects the specified limit"
        : `Expected depth <= 2, got ${response.cascade.metadata.depth}`,
    });
  } catch (error) {
    tests.push({
      name: "Depth N respects limit",
  ...
  try {
    const response = await executeMutation(server, "createUser", {
      name: "Test User",
      email: "test@example.com",
    });
    const hasDepth =
      typeof response.cascade.metadata.depth === "number" &&
      response.cascade.metadata.depth > 0;
    tests.push({
      name: "Default depth applied",
      passed: hasDepth,
      message: hasDepth
        ? "Default depth is applied in metadata"
        : "No valid depth found in cascade metadata",
    });
  } catch (error) {
    tests.push({
      name: "Default depth applied",
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "Max depth limit is properly enforced"
        : `Expected depth <= 3, got ${response.cascade.metadata.depth}`,
    });
  } catch (error) {
    tests.push({
      name: "Max depth enforced",
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "Depth metadata matches actual cascade depth"
        : `Expected depth ${expectedDepth}, got ${actualDepth}`,
    });
  } catch (error) {
    tests.push({
      name: "Depth metadata accurate",
 ...
 */
async function runRelationshipTraversalTests(
  options: ServerConformanceOptions,
): Promise<TestCategory> {
  const tests: TestResult[] = [];
  const server = options.createServer?.() || mockTestServer();
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "One-to-one relationship properly tracked in cascade"
        : "Missing User or Profile in cascade updates",
    });
  } catch (error) {
    tests.push({
      name: "One-to-one tracked",
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "One-to-many relationship properly tracked in cascade"
        : `Expected 1 User and 2 Posts, got ${userUpdates.length} Users and ${postUpdates.length} Posts`,
    });
  } catch (error) {
    tests.push({
      name: "One-to-many tracked",
  ...
  try {
    ...
    const passed =
      ...
      groupUpdates.length === 2 &&
      membershipUpdates.length === 2;
    tests.push({
      name: "Many-to-many tracked",
      passed,
      message: passed
        ? "Many-to-many relationship properly tracked in cascade"
        : `Expected 1 User, 2 Groups, 2 Memberships; got ${userUpdates.length}/${groupUpdates.length}/${membershipUpdates.length}`,
    });
  } catch (error) {
    tests.push({
      name: "Many-to-many tracked",
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "Circular references handled without duplication"
        : `Expected 2 Users, got ${userUpdates.length}`,
    });
  } catch (error) {
    tests.push({
      name: "Circular refs handled",
  ...
  try {
    const response = await executeMutation(
      ...
      "createCategoryTree",
      {
        categories: [
          {
            name: "Parent",
            children: [{ name: "Child", parentId: "parent-id" }],
          },
        ],
      },
      { depth: 2 },
    ...
    tests.push({
      ...
      message: passed
        ...
        : `Expected 2 Categories, got ${categoryUpdates.length}`,
    });
  } catch (error) {
    tests.push({
      name: "Self-referential handled",
  ...
  try {
    const response = await executeMutation(
      ...
      "createPostWithComments",
      {
        ...
        comments: [
          {
            text: "Comment 1",
            author: { name: "Commenter 1", email: "c1@example.com" },
          },
          {
            text: "Comment 2",
            author: { name: "Commenter 2", email: "c2@example.com" },
          },
        ],
      },
      { depth: 3 },
    ...
    const passed =
      ...
      postUpdates.length === 1 &&
      commentUpdates.length === 2;
    tests.push({
      name: "Nested creates tracked",
      passed,
      message: passed
        ? "Nested creates properly tracked in cascade"
        : `Expected 3 Users, 1 Post, 2 Comments; got ${userUpdates.length}/${postUpdates.length}/${commentUpdates.length}`,
    });
  } catch (error) {
    tests.push({
      name: "Nested creates tracked",
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "Nested updates properly tracked in cascade"
        : `Expected 1 Post update, 2 Comment updates; got ${postUpdates.length}/${commentUpdates.length}`,
    });
  } catch (error) {
    tests.push({
      name: "Nested updates tracked",
  ...
  try {
    ...
    tests.push({
      ...
      passed,
      message: passed
        ? "Cascade correctly stops at depth limit"
        : `Expected depth <= 3, got ${response.cascade.metadata.depth}`,
    });
  } catch (error) {
    tests.push({
 ...
 */
async function runTransactionMetadataTests(
  options: ServerConformanceOptions,
): Promise<TestCategory> {
  const tests: TestResult[] = [];
  const server = options.createServer?.() || mockTestServer();
  ...
  try {
    ...
    const passed =
      typeof hasTransactionId === "string" && hasTransactionId.length > 0;
    tests.push({
      name: "Transaction ID included",
      passed,
      message: passed
        ? "Transaction ID is included in cascade metadata"
        : "Missing transaction ID in cascade metadata",
    });
  } catch (error) {
    tests.push({
      name: "Transaction ID included",
  ...
  try {
    const response1 = await executeMutation(server, "createUser", {
      name: "User 1",
    ...
    const passed = Boolean(
      transactionId1 && transactionId2 && transactionId1 !== transactionId2,
    );
    tests.push({
      name: "Transaction ID consistent",
      passed,
      message: passed
        ? "Different operations have different transaction IDs"
        : "Transaction IDs should be unique per operation",
    });
  } catch (error) {
    tests.push({
      name: "Transaction ID consistent",
  ...
  try {
    ...
    const isRollback =
      !response.success &&
      response.cascade.updated.length === 0 &&
      response.cascade.metadata.affectedCount === 0;
    const passed = isRollback;
    tests.push({
      name: "Rollback empty cascade",
      passed,
      message: passed
        ? "Failed operations result in empty cascade (rollback)"
        : "Failed operations should result in empty cascade",
    });
  } catch (error) {
    tests.push({
      name: "Rollback empty cascade",
  ...
  try {
    const response = await executeMutation(server, "createPostWithComments", {
      ...
      comments: [
        {
          text: "Valid Comment",
          author: { name: "Commenter", email: "commenter@example.com" },
        },
    ...
    const hasWarnings =
      response.cascade.metadata.warnings &&
      response.cascade.metadata.warnings.length > 0;
    const passed = hasPartialSuccess && hasWarnings;
    tests.push({
      name: "Partial success indicated",
      passed,
      message: passed
        ? "Partial success is properly indicated with warnings"
        : "Partial success should be indicated when some operations fail",
    });
  } catch (error) {
    tests.push({
      name: "Partial success indicated",
                          ...
                          ? [
                              {
                                ...
                                author:
                                  effectiveDepth >= 4
                                    ? {
                                        id: "2",
                                        name: variables.posts[0].comments[0]
                                          .author.name,
                                      }
                                    : undefined,
                              },
