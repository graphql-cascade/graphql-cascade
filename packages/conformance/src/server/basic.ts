...
function createEntityTrackingTests(): BasicTestCase[] {
  return [
      ...
      test: () => {
        ...
        return {
          ...
          message: hasPrimaryEntity
            ? "Primary entity is tracked in updated array"
            : "Primary entity not found in updated array",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        const createdEntity = cascade.updated.find(
          (entity: any) => entity.operation === "CREATED",
        );
        return {
          name: "Created entity in updated array",
          passed: !!createdEntity,
          message: createdEntity
            ? "Created entity found in updated array"
            : "No created entity in updated array",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        const updatedEntity = cascade.updated.find(
          (entity: any) => entity.operation === "UPDATED",
        );
        return {
          name: "Updated entity in updated array",
          passed: !!updatedEntity,
          message: updatedEntity
            ? "Updated entity found in updated array"
            : "No updated entity in updated array",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          message: deletedEntity
            ? "Deleted entity found with deletedAt timestamp"
            : "No deleted entity in deleted array",
        };
      },
    },
    {
    ...
    },
    {
      ...
      test: () => {
        const cascade = successResponse.data.createUser.cascade;
        const hasCorrectId = cascade.updated.every(
          (entity: any) =>
        ...
        return {
          name: "Correct id",
          passed: hasCorrectId,
          message: hasCorrectId
            ? "All entities have correct id"
            : "Some entities have incorrect id",
        };
      },
    },
    {
      ...
      test: () => {
        const cascade = successResponse.data.createUser.cascade;
        const hasCorrectOperation = cascade.updated.every(
          (entity: any) => entity.operation === "CREATED",
        );
        return {
          name: "Correct operation",
          passed: hasCorrectOperation,
          message: hasCorrectOperation
            ? "All entities have correct operation"
            : "Some entities have incorrect operation",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          passed: !!matches,
          message: matches
            ? "Entity data matches committed state"
            : "Entity data does not match committed state",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        const affectedCountMatches =
          cascade.metadata.affectedCount === cascade.updated.length;
        return {
          name: "Multiple entities tracked",
          passed: multipleTracked && affectedCountMatches,
          message: multipleTracked
            ? "Multiple entities tracked with correct count"
            : "Not enough entities tracked",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          message: isEmpty
            ? "Read-only query has empty cascade"
            : "Read-only query has unexpected cascade data",
        };
      },
    },
  ];
  ...
  return [
    {
      ...
      test: () => {
        const cascade = successResponse.data.createUser.cascade;
        const hasUsersInvalidation = cascade.invalidations.some(
          (inv: any) => inv.queryName === "users",
        );
        return {
          name: "Create invalidates list queries",
          passed: hasUsersInvalidation,
          message: hasUsersInvalidation
            ? "Create operation invalidates list queries"
            : "Create operation does not invalidate list queries",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        const hasRelatedInvalidations = cascade.invalidations.some(
          (inv: any) => inv.scope === "RELATED",
        );
        return {
          name: "Update may invalidate related",
          passed: hasRelatedInvalidations,
          message: hasRelatedInvalidations
            ? "Update invalidates related queries"
            : "No related invalidations found",
        };
      },
    },
    {
      ...
      test: () => {
        const cascade = deleteResponse.data.deleteUser.cascade;
        const hasRemoveStrategy = cascade.invalidations.some(
          (inv: any) => inv.strategy === "REMOVE",
        );
        const hasExactScope = cascade.invalidations.some(
          (inv: any) => inv.scope === "EXACT",
        );
        return {
          name: "Delete invalidates entity queries",
          passed: hasRemoveStrategy && hasExactScope,
          message:
            hasRemoveStrategy && hasExactScope
              ? "Delete properly invalidates entity queries"
              : "Delete does not properly invalidate entity queries",
        };
      },
    },
    {
      ...
      test: () => {
        const cascade = successResponse.data.createUser.cascade;
        const hasQueryName = cascade.invalidations.every(
          (inv: any) => inv.queryName,
        );
        return {
          name: "Has queryName",
          passed: hasQueryName,
          message: hasQueryName
            ? "All invalidations have queryName"
            : "Some invalidations missing queryName",
        };
      },
    },
    {
      ...
      test: () => {
        const cascade = successResponse.data.createUser.cascade;
        const hasStrategy = cascade.invalidations.every(
          (inv: any) => inv.strategy,
        );
        return {
          name: "Has strategy",
          passed: hasStrategy,
          message: hasStrategy
            ? "All invalidations have strategy"
            : "Some invalidations missing strategy",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          passed: hasScope,
          message: hasScope
            ? "All invalidations have scope"
            : "Some invalidations missing scope",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          passed: hasMultiple,
          message: hasMultiple
            ? `${cascade.invalidations.length} invalidations generated`
            : "Only one or zero invalidations",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        const unique = new Set(
          invalidations.map(
            (inv: any) => `${inv.queryName}-${inv.strategy}-${inv.scope}`,
          ),
        );
  ...
  return [
    ...
    },
    {
      ...
      test: () => {
        const metadata = successResponse.data.createUser.cascade.metadata;
        const hasDepth =
          metadata.depth !== undefined && metadata.depth !== null;
        return {
          name: "Has depth",
          passed: hasDepth,
    ...
    },
    {
      ...
      test: () => {
        ...
        const hasAffectedCount =
          ...
          metadata.affectedCount !== null;
        return {
          name: "Has affectedCount",
          passed: hasAffectedCount,
          message: hasAffectedCount
            ? "Metadata has affectedCount"
            : "Metadata missing affectedCount",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          passed: accurate,
          message: accurate
            ? "affectedCount matches actual affected entities"
            : `affectedCount ${metadata.affectedCount} does not match actual ${actualAffected}`,
      ...
      test: () => {
        ...
        return {
          ...
          message: isValidISO
            ...
            : `Timestamp ${timestamp} is not valid ISO 8601`,
        };
      },
    },
  ];
      ...
      test: () => {
        ...
        return {
          ...
          message: hasValidationError
            ? "Error has VALIDATION_ERROR code"
            : "Error missing VALIDATION_ERROR code",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          passed: hasNotFound,
          message: hasNotFound
            ? "Error has NOT_FOUND code"
            : "Error missing NOT_FOUND code",
        };
      },
    },
    {
    ...
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          passed: hasField,
          message: hasField
            ? "Error has field property"
            : "Error missing field property",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          passed: hasPath,
          message: hasPath
            ? "Error has path property"
            : "Error missing path property",
        };
      },
    },
    {
      ...
      test: () => {
        ...
        return {
          ...
          message: hasSuccessFalse
            ? "Failed mutation has success: false"
            : "Failed mutation does not have success: false",
        };
      },
    },
  ];
