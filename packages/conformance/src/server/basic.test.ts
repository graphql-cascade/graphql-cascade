...
describe("runBasicTests", () => {
  ...
  it("should include Entity Tracking category", async () => {
    const categories = await runBasicTests(options);
    const entityTracking = categories.find(
      (cat) => cat.name === "Entity Tracking",
    );
  ...
  it("should include Invalidation Hints category", async () => {
    const categories = await runBasicTests(options);
    const invalidationHints = categories.find(
      (cat) => cat.name === "Invalidation Hints",
    );
  ...
  it("should include Error Handling category", async () => {
    const categories = await runBasicTests(options);
    const errorHandling = categories.find(
      (cat) => cat.name === "Error Handling",
    );
  ...
  it("should pass basic fixture-based tests", async () => {
    ...
    const primaryTracked = entityTracking?.tests.find(
      (t) => t.name === "Primary mutation result tracked",
    );
    ...
    const metadata = categories.find((cat) => cat.name === "Metadata");
    const hasTimestamp = metadata?.tests.find(
      (t) => t.name === "Has timestamp",
    );
    ...
    const validationError = errorHandling?.tests.find(
      (t) => t.name === "VALIDATION_ERROR code",
    );
