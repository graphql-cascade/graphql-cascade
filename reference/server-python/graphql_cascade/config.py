...
class CascadeConfig:
    ...
    def from_env(cls) -> 'CascadeConfig':
        ...
        if 'CASCADE_TRACK_RELATIONSHIPS' in os.environ:
            cascade_config['enable_relationship_tracking'] = os.environ['CASCADE_TRACK_RELATIONSHIPS'].lower() == 'true'

        # Response settings
        response_config = {}
        if 'CASCADE_MAX_RESPONSE_SIZE_MB' in os.environ:
            response_config['max_response_size_mb'] = float(os.environ['CASCADE_MAX_RESPONSE_SIZE_MB'])

        if 'CASCADE_MAX_UPDATED_ENTITIES' in os.environ:
            response_config['max_updated_entities'] = int(os.environ['CASCADE_MAX_UPDATED_ENTITIES'])

        if 'CASCADE_MAX_DELETED_ENTITIES' in os.environ:
            response_config['max_deleted_entities'] = int(os.environ['CASCADE_MAX_DELETED_ENTITIES'])
        ...
        if 'CASCADE_AUTO_INVALIDATE' in os.environ:
            invalidation_config['auto_compute'] = os.environ['CASCADE_AUTO_INVALIDATE'].lower() == 'true'

        if 'CASCADE_INCLUDE_RELATED_INVALIDATIONS' in os.environ:
            invalidation_config['include_related'] = os.environ['CASCADE_INCLUDE_RELATED_INVALIDATIONS'].lower() == 'true'
